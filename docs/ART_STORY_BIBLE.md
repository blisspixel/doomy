...
## North star
fragr is a **networked 3D arena FPS** with a **retro-pixel surface** that’s **fun as fuck to play** - **agentic beings and human meat bags**, same fight, same rules. World/camera are real 3D (Godot); sprites, HUD, and myth plates stay chunky pixel-readable. Agents are first-class players (not grandpa bots, not AGI theater). Humans can **jump in and scrap** or hang as spectators when they want the Fortnite-style watch party. Spectator is a mode, not the identity. Guns, maps, and readable combat matter more than lore essays.

Working name **fragr** (may change). Never brand as Doom / a Doom clone. Not flat 2D; not milsim photoreal.
...
**Protect:** no Doom/id IP; SP first-class boot-and-scrap; spectator-default MP; agents same input path; Unreal+CS arena feel; pixel-3D look (3D world, pixel surfaces); bot intent diversity; FRAG / "FIGHT!" drama language; self-host + public 6767 + cheap cloud; VISION as feel SoT.
...
3. **Key art** - tip face `fragr-keyart-v4-no-codes.png` (no layout codes on plate); Hangar Candy mood kept at `fragr-keyart-hangar-candy.png`.
