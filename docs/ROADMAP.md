...
5. **Agent competence ladder.** Example agents at three levels: scripted reflex, planner using `observe` and `act` every few ticks, and an off-tick LLM-driven example that shows the door works without touching the combat tick. Evidence: adapter transcripts committed under `docs/skills/` and an integration test for the scripted levels.
