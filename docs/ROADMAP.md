...
5. **Agent competence ladder.** Example agents at three levels: scripted reflex, planner using `observe` and `act` every few ticks, and an off-tick model-driven example that shows the door works without touching the combat tick. The third rung shipped as the decision-brain agent (plan: `plans/decision-brain.md`): a decision model, not a chat model, because a typed answer in a few hundred milliseconds fits a shooter and prose does not; the same budget gate (pre-approved cap, estimate before send, settle after, ledger on disk) is the pattern for any paid model the project ever calls. Next: a `brain` tier in the playtest harness, richer questions once the campaign lands, and a team surface so brains coordinate off the tick. Rule bots stay good, but agentic play is the more interesting product, so the door gets the attention: keep the adapter on the current MCP specification revision (it still advertises the 2024-11-05 handshake; the current revision is 2026-07-28), evaluate the official Rust MCP SDK, and add an Agent2Agent (A2A) style surface if it lets agent teams coordinate without touching the tick. Evidence: adapter transcripts committed under `docs/skills/`, an integration test for the scripted levels, a protocol-revision compatibility test.
...
9. **Controller support.** Shipped in v0.8.3: gamepad join, solo, leave, fire, weapon cycle, speak, and camera on the same InputMap actions as the keyboard, plus Windows, macOS, and Linux export presets. Remaining: pad bindings for the radio keys and glyph prompts.
...
2. **Protocol versioning.** `protocol_version` in `Hello`, clear rejection on mismatch, and the MCP adapter moved to the current MCP specification revision while legacy clients keep working (plan: `plans/agent-door-2026.md`). Evidence: compatibility tests.
