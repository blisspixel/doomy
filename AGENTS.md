...
- **Spend:** hard cap **$50** total. Local play and LAN are $0. Anything that bills (cloud apply, VPS, paid assets, paid model APIs) needs written approval from Nick first. Approved paid APIs: ElevenLabs, used only by developers through `tools/audiogen`; and the Jev decision model (TypeSafe native or OpenRouter) used only through `agents/brain`, which refuses to start a paid provider without an explicit `--max-spend-usd` cap (developer ceiling 5 dollars per run) and ledgers every call under `.agents/spend/`. Neither runs in CI or at player runtime by default. No other paid model or vision API without approval.
...
- **Languages:** Rust and GDScript only. No Python or other scripting languages in tooling.
